...
//         const response = await axios.get(`https://backend-herbal.onrender.com/user/63687312-b14e-400c-9afe-af49db794cc8/single_user`); // Adjust the endpoint accordingly
          ...
          <tbody>
            {orders.length > 0 ? (
              orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  <td className="py-2 px-4 border-b">{order.id}</td>
                  <td className="py-2 px-4 border-b">${parseFloat(order.totalAmount).toFixed(2)}</td>
                  <td className="py-2 px-4 border-b">${parseFloat(order.deliveryFee).toFixed(2)}</td>
