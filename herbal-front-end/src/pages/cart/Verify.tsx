  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      try {
        ...
        if (response.data.status === 'success') {
          ...
          // Only create the order if the payment verification is successful
          await createOrder();
        } else {
          setPaymentStatus(null); // Payment verification failed
        }
      } catch (error) {
        console.error('Payment verification failed:', error);
        setPaymentStatus(null);
    ...
    const createOrder = async () => {
      ...
      try {
        ...
        if (orderResponse.status === 200) {
          clearCart(); // Clear cart after the order is successfully created
        }
      } catch (orderError) {
        console.error('Order creation failed:', orderError);
        setOrderError('Failed to create order. Please try again.');
    ...
    verifyPayment();
  }, [reference]); // Reduced dependencies to avoid multiple runs

  // Redirect to products page after 1 minute if payment was successful
  useEffect(() => {
    if (paymentStatus) {
      const timer = setTimeout(() => {
        navigate('/products');
